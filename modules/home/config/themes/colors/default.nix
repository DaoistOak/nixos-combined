...
let
  ...
  # `toBase16` reads `name` off the roles record; readSelection attaches the
  # same field to its own copy, so mirror it here.
  ...
  # --- neovim runtime theme file (base16 palette read by lua/theme.lua) -------
  # scripts/theme rewrites this file and sends SIGUSR1 to running instances, so
  # `theme set ...` recolors nvim without a rebuild. Slot order must match
  # themes.nix `toBase16` (Stylix consumes the same mapping).
  nvimBase16 =
    let
      b = toBase16 r;
    in
    ''
      ...
      return {
      ${lib.concatStringsSep "\n" (
        map (k: "  ${k} = \"${b.${k}}\",") (lib.filter (k: lib.hasPrefix "base" k) (lib.attrNames b))
      )}
      }
    '';

  # --- hyprland runtime theme file (lua fragment, dofiled by extraConfig) -----
  # Returns { active_border, inactive_border } in 0xRRGGBB format. active_border
  # is a 2-colour gradient (theme secondary -> primary, flipped relative to the
  # rest of the theme, for the border only) that the border-angle daemon
  # (hypr/scripts/border-angle.sh) rotates toward the cursor at runtime.
  # colors.nix (extraConfig) dofiles this on every hyprctl reload so switching
  # needs no rebuild.
  hyprlandTheme =
    let
      x = v: "0x${v}";
      # scripts/theme resolves `secondary` as the ANSI blue slot (ansi[4]);
      # mirror that here so the build-time and runtime files emit the same pair.
      secondary = builtins.elemAt r.ansi 4;
    in
    ''
      -- Generated by modules/config/colors (default) / scripts/theme (runtime).
      -- Theme: ${defaultTheme} / ${defaultFlavor} / ${defaultAccent}
      return {
        active_border = { c1 = "${x secondary}", c2 = "${x r.accent}", angle = 0 },
        inactive_border = "${x r.base}",
      }
    '';
in
{
  ...
  config = {
    ...
    home.file.".config/theme-switcher/nvim-base16.lua" = {
      text = nvimBase16;
      force = true;
    };
    ...
    home.activation.themeSwitcherWritable =
      lib.hm.dag.entryAfter
        ...
        ''
          ...
          for f in kitty-theme.conf alacritty-theme.toml wezterm-theme.lua ghostty-theme.conf tmux-theme.conf hyprland-theme.lua nvim-base16.lua; do

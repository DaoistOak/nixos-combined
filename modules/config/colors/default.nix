...
let
  ...
  # --- tmux runtime theme file (colors for the native pill status bar) ---------
  ...
  tmuxTheme =
    let
      c = v: "#${v}";
      # module pill colors
      bu = builtins.elemAt r.ansi 4; # blue  -> battery
      yw = builtins.elemAt r.ansi 3; # yellow-> cpu
      te = builtins.elemAt r.ansi 6; # cyan  -> uptime
    in
    ''
      # Generated by modules/config/colors (default) / scripts/theme (runtime).
      # Theme: ${defaultTheme} / ${defaultFlavor} / ${defaultAccent}
      ...
      set -g mode-style "bg=${c r.accent},fg=${c r.base}"
      set -g display-panes-colour "${c r.accent}"
      set -g display-panes-active-colour "${c r.text}"
      # Pill status bar colors
      set -gq @thm_tab_bg "${c r.surface0}"
      set -gq @thm_tab_fg "${c r.text}"
      set -gq @thm_tab_accent "${c r.accent}"
      set -gq @thm_tab_inactive "${c r.overlay2}"
      set -gq @thm_on_pill "${c r.base}"
      set -gq @thm_m_user "${c r.accent}"
      set -gq @thm_m_battery "${c bu}"
      set -gq @thm_m_cpu "${c yw}"
      set -gq @thm_m_uptime "${c te}"
    '';

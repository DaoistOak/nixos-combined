...
}:
{
  programs.tmux = {
    ...
    extraConfig = ''
      ...
      set -g window-status-current-format "#[fg=#{@thm_accent},bg=#{@thm_bg}]#[fg=#{@thm_bg},bg=#{@thm_accent}]#{window_index} #[fg=#{@thm_fg},bg=#{@thm_accent}] #{=40:#{window_name}}#[fg=#{@thm_surface_0},bg=#{@thm_bg}]"
